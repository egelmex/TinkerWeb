...
module Jekyll
  class FlickrSetTag < Liquid::Tag
    ...
    def render(context)
      if @config['gallery_id'] == nil
        html = "<#{@config['gallery_tag']} class=\"#{@config['gallery_class']}\">"
      else
        html = "<#{@config['gallery_tag']} id=\"#{@config['gallery_id']}\" class=\"#{@config['gallery_class']}\">"
      end

      photos.each do |photo|
        html << "<a href=\"#{photo.url(@config['a_href'])}\" target=\"#{@config['a_target']}\">"
        html << "  <img src=\"#{photo.thumbnail_url}\" rel=\"#{@config['image_rel']}\" class=\"#{@config['image_class']}\"/>"
        html << "</a>"
      end
